...
import pandas as pd
import traceback
from utils.directroy_util import directory_month_pick_day_read
...
from chart.pick_day_chart import pick_day_chart_valued_sort

def csv_pick_day(display_name, year, month, target, day_pick_list):
    day_pick_list = [f"{year}-{month}-{day}" for day in day_pick_list]
    json_file_path = directory_month_pick_day_read(display_name, year, month, day_pick_list)
    ...
    # 에러 파일 json 파일 저장
    json_file_path = f'{processed_path}/error_csv_day_{display_name}_{display_id}_{year}_{month}_pick_day.json'
    with open(json_file_path, 'w', encoding='utf-8') as f:
        json.dump(error_file_dict, f, ensure_ascii=False, indent=4) 
        
    # 전처리 데이터 파일 저장
    json_file_path = f'{processed_path}/csv_day_{display_name}_{display_id}_{year}_{month}_{target}_pick_day.json'
    with open(json_file_path, 'w', encoding='utf-8') as f:
        json.dump(csv_day_dict, f, ensure_ascii=False, indent=4) 
    pick_day_chart_valued_sort(csv_day_dict,target,year,display_name,month)

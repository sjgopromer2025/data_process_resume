...
def day_chart_valued_sort(csv_day_dict, target, year, display_name, month):
...
def bar_chart_image_save(zipped_data, target, month, year, display_name, target_title):
    ...
    plt.ylabel("Count")
    plt.title(
        f"{display_name} {target_title} {get_month_name(month)} Count per Day (Sorted)"
    )
    # plt.ylim(min_count - 100, max_count + 100)  # 여유를 두고 설정
    plt.xticks(rotation=85)
    plt.tight_layout()

    image_name = f"{display_name}_{year}_{month}_{target_title}_day_chart.png"
    # 이미지 파일로 저장
    image_save_path = os.path.join(
        report_path, display_name, "day", year, month, target
    )

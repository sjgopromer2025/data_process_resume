...
def rank_chart_valued_sort(csv_rank_dict, target, year, display_name, month):
    # print(json_data,display_id,target,year,display_name,target_title)
    # zip을 사용하여 keys와 counts 분리
    # display_ids, counts = zip(*[(key, value['count']) for key, value in json_data.items()])

    display_ids = list(csv_rank_dict.keys())

    if target == target_list[0]:
        counts = []
        for display_id in display_ids:
            counts.append(csv_rank_dict[display_id]["count"])
        ...
        for display_id in display_ids:
            ...
            male_count.append(csv_rank_dict[display_id]["M"])
            female_count.append(csv_rank_dict[display_id]["F"])
        ...
        zip_for_age(csv_rank_dict, target, year, display_name, month)
    elif target == target_list[3]:
        zip_for_age_gender(csv_rank_dict, target, year, display_name, month)
...
def bar_chart_image_save(zipped_data, target, month, year, display_name, target_title):
    ...
    image_name = f"{display_name}_{year}_{month}_{target_title}_rank_chart.png"
    # 이미지 파일로 저장
    image_save_path = os.path.join(
        report_path, display_name, "rank", year, month, target
    )
...
def zip_for_age(csv_rank_dict, target, year, display_name, month):
    ...
    for display_id in display_ids:
        age_0_20_counts.append(csv_rank_dict[display_id]["age_0_20"])
        age_20_40_counts.append(csv_rank_dict[display_id]["age_20_40"])
        age_40_60_counts.append(csv_rank_dict[display_id]["age_40_60"])
        age_60_above_counts.append(csv_rank_dict[display_id]["age_60_above"])
...
def zip_for_age_gender(csv_rank_dict, target, year, display_name, month):
    display_ids = list(csv_rank_dict.keys())
    # 연령대-성별별 리스트 초기화
    ...
    for display_id in display_ids:
        age_0_20_male_counts.append(csv_rank_dict[display_id]["age_0_20_Male"])
        age_0_20_female_counts.append(csv_rank_dict[display_id]["age_0_20_Female"])
        age_20_40_male_counts.append(csv_rank_dict[display_id]["age_20_40_Male"])
        age_20_40_female_counts.append(csv_rank_dict[display_id]["age_20_40_Female"])
        age_40_60_male_counts.append(csv_rank_dict[display_id]["age_40_60_Male"])
        age_40_60_female_counts.append(csv_rank_dict[display_id]["age_40_60_Female"])
        age_60_above_male_counts.append(csv_rank_dict[display_id]["age_60_above_Male"])
        age_60_above_female_counts.append(
            csv_rank_dict[display_id]["age_60_above_Female"]
        )
